...
**0a — Parse arguments.**
Split `$ARGUMENTS`. First token = `FOCUS`. Valid values: `docs`, `enforcement`, `xml`, `counts`, `rag`, `all`. Default to `all` if omitted or unrecognized.
...
**PHASE 3 GATE:** If more than 3 self-tests FAIL, STOP. Print: "Self-test gate: N failures detected. Fix ST failures before proceeding to audit." Investigate the failures before continuing.
