...
description: Uninstall ClaudeBoost, reverses scripts/setup.py (hooks, env, symlinks, MCP, RAG server). --purge also removes pip package, indexes, PATH edits, shared MCPs
allowed-tools: Bash, Read, AskUserQuestion
---

# /uninstall: Remove ClaudeBoost ($ARGUMENTS)

Reverses everything the installer (`scripts/setup.py`) installed. The work is done by `scripts/uninstall.py`;
...
reversible by re-running `scripts/setup.py`:
...
4. **Confirm before changing anything.** Uninstalling edits the user's *global*
   ...
   > the RAG server (re-runnable via `scripts/setup.py`). Proceed?"
...
6. **Closing note.** Tell the user to **restart any open Claude Code sessions** so they
   ...
   if they want to delete it. Mention that re-running `scripts/setup.py` reinstalls everything if they change their mind.

## Notes

- The default path is reversible: `scripts/setup.py` reinstalls the full footprint.
- `--purge` is not reversible for free, the RAG index has to be rebuilt with
  `/index-project`, and shared MCPs re-registered by `scripts/setup.py`.
