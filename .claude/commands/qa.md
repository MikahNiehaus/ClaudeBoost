---
argument-hint: [url | --code | file-path | workspace-id | "description of what to QA"] [scope — auth | crud | nav | errors | responsive | all] [--no-debug] [--no-video] [--fresh]
description: Full QA session — works on anything. Browser apps (pass a URL), code changes (--code or file paths), scripts, artifacts, workspace output. Builds inventory from RAG, writes a risk-prioritized test plan, executes with evidence, checks the proof with a subagent before calling it done, and reports what was tested AND what was not. Ends by building a narrated video walkthrough of the session actually doing the work, cut to the test case segments (still-image deck as fallback when nothing could be recorded).
...
- `--no-video` present → set `NO_VIDEO = true` (skip recording pre-flight entirely in Phase 0h; Phase 7 takes its 7f still-image fallback)
...
│       ├── TC-001-after.png
│       └── TC-001-before.png
├── recording/              (the session recording, its timeline and cut report)
│   ├── session.mp4         (path `window`) or a generated .webm (path `context`)
│   ├── timeline.tsv        (TC start/end epochs — what Phase 7 cuts on)
│   ├── stop_epoch          (when recording stopped, written once in Phase 5e)
│   ├── narration.json      (spoken text per TC, written in Phase 7b)
│   ├── video-report.json   (per segment in/out points and the skipped list)
│   └── verify-frames/      (one mid-segment frame per TC — read every one)
...
## Phase 0h: Recording Pre-flight (browser mode only — runs before Phase 1)

The primary proof artifact of a browser-mode session is a **video of the session
actually doing the work**, not a set of stills. Everything in this phase exists
because both available recording paths fail silently. A session that assumes it
is recording and is not produces zero video proof and does not find out until
Phase 7, after the browser has closed and the evidence is unrecoverable.

**`--no-video` skip gate:** if `NO_VIDEO = true` (flag was passed in arguments),
set `RECORDING = none`, print "Video recording skipped (`--no-video`).
Screenshots only.", and go to Phase 1. Phase 7 then takes its still-image
fallback at 7f.

**0h-i — Dependency check.**

```bash
python "${HOME}/.claude/skills/powerpoint/scripts/pptx_env.py" doctor
```

`ffmpeg` and `edge-tts` gate this phase. If either is missing: set
`RECORDING = none`, print which one and the install command doctor gave, and
continue to Phase 1 without video. Do not fail the session over it.

Set `RECORD_DIR = $WORKSPACE_ABS/recording` and create it.

**0h-ii — Pick the recording path.**

Two exist. Neither is a default you can assume.

| Path | What it films | Cost |
|---|---|---|
| `context` (higher fidelity) | the page itself, via Playwright's own `recordVideo` | needs an MCP config change and a session restart |
| `window` (works immediately) | the browser window as composited on screen, via ffmpeg `gdigrab` | black-frame and occlusion risk |

**Path `context`.** Playwright MCP has no supported video flag. Video is an open
feature request (microsoft/playwright-mcp#695, where a PR was converted into a
tracking issue rather than merged), and `--save-video` is reported broken in
#1134 and #1093. The one configuration that works is undocumented and nested
(#1506): the documented top-level `saveVideo` property does not map through to
the browser context at all.

Check whether it is already in effect:

```bash
python "${CLAUDEBOOST_HOME}/scripts/show-playwright-mcp-config.py"
```

If no such helper exists, read the `playwright` entry's `args` out of
`~/.claude.json` with a one-line `python -c` using `json.load`. If the args
already contain `--config` pointing at a file whose
`browser.contextOptions.recordVideo.dir` is set, set `RECORDING = context`,
point that dir at `$RECORD_DIR`, and go to 0h-iii.

If not, write the config file and **hand the change to the user**. This is an
edit to their MCP server configuration and it needs a session restart to take
effect, so it is theirs to make. Write:

```json
{
  "browser": {
    "contextOptions": {
      "recordVideo": { "dir": "<RECORD_DIR>", "size": { "width": 1280, "height": 800 } }
    }
  }
}
```

to `~/.claude/playwright-mcp-video.json`, then tell them the one line to add to
the playwright entry's `args` (`"--config", "<that path>"`), that it needs a
restart, and that until then this session falls back to path `window`. Do not
edit `~/.claude.json` yourself.

**Three sizes have to agree, or the capture is padded, scaled, or both.** Getting
any one of them wrong produces a video that is technically valid and unusable,
and none of the builder's numeric checks can see it:

| Setting | Must be | What goes wrong otherwise |
|---|---|---|
| the viewport | the size you want to record | — |
| `recordVideo.size` | **the same as the viewport** | Left unset it defaults to **800x600**, and a larger page gets squeezed into it: the aspect mismatch letterboxes the frame and the downscale blurs every glyph |
| the device scale factor | `1` | A display at 125% or 150% Windows scaling gives `devicePixelRatio` 1.25 or 1.5, so Chrome rasterises at that factor and the result is resampled into the video. Force it with the `--force-device-scale-factor=1` browser arg |

Also make the OS window **larger** than the viewport, never smaller. A window
smaller than the viewport makes Chrome render the page padded inside the capture
canvas, in a corner, with dead space around it.

Confirm it rather than assuming, once the page is loaded and before the first
test case:
...
and probe the written file's real dimensions with
`ffprobe -show_entries stream=width,height`. A source that is not the size you
asked for is the single most likely reason a finished video is unwatchable.

**Path `window`.** Start ffmpeg against the browser window title *after* Phase 1a
has opened it, not before. The window must exist first. Two failure modes, both
real, both of which silently produce useless evidence:

- Chrome composites through the GPU, so `gdigrab` window capture often yields
  **all-black frames**. The 0h-iii probe is the only thing that catches this.
- Capturing `-i desktop` instead would record everything else on the user's
  screen, including windows that have nothing to do with this session. Never do
  that. Window title capture only.

```bash
ffmpeg -hide_banner -loglevel warning -y \
  -f gdigrab -framerate 10 -draw_mouse 0 -i title="<exact browser window title>" \
  -c:v libx264 -crf 20 -preset veryfast -pix_fmt yuv420p \
  "$RECORD_DIR/session.mp4"
```

Run it with `run_in_background: true`. Set `RECORDING = window`.

`-draw_mouse 0` is deliberate. The real OS pointer never moves during a driven
session, so drawing it would film a stationary arrow in one corner and imply the
session was hand-driven. The cursor that matters is the injected one from 0h-iv.

**0h-iii — Probe that recording is real. Mandatory.**

Run this after Phase 1a's first navigation, before any test case.

- Path `window`: wait 3 seconds, then extract a frame and look at it.
  ```bash
  ffmpeg -hide_banner -loglevel error -y -sseof -1 -i "$RECORD_DIR/session.mp4" -frames:v 1 "$RECORD_DIR/probe.png"
  ```
  `Read` `probe.png`. If it is black, blank, or not the browser, path `window`
  has failed. Stop the ffmpeg job, set `RECORDING = none`, and print that video
  proof is unavailable this session with the reason. Do not carry on as if it
  were recording.
- Path `context`: Playwright flushes a `recordVideo` file only when the context
  closes, so there is nothing on disk to probe mid-session. Confirm instead that
  the directory exists and is writable, and treat Phase 7a's "no recording file"
  branch as the real check.

Print one line either way:
```
Recording: [context | window | none] → $RECORD_DIR   (probe: [frame OK | black frames | deferred to Phase 7])
```

**0h-iv — Inject the cursor overlay.**

A driven browser has no visible pointer. Synthetic input does not move the OS
cursor, so an unmodified recording shows fields filling and buttons depressing
with nothing pointing at them, which is the exact thing a viewer needs to see.

The overlay tracks real DOM `mousemove` events, which Playwright's own click and
hover do dispatch. Adapted from `cenfun/mouse-helper` (MIT), reduced to a
dependency-free snippet. Inject with `browser_evaluate`:

```javascript
(function(){
  if (document.getElementById('__e2e_cursor__')) return 'already';
  var c = document.createElement('div');
  c.id = '__e2e_cursor__';
  c.style.cssText = 'position:fixed;top:0;left:0;width:22px;height:22px;'
    + 'margin:-11px 0 0 -11px;border-radius:50%;z-index:2147483647;'
    + 'pointer-events:none;background:rgba(255,60,0,.35);'
    + 'border:2px solid #FF3C00;box-shadow:0 0 0 2px rgba(255,255,255,.9);'
  ...
  document.addEventListener('mousedown', function(){
    ...
    c.style.background = 'rgba(255,60,0,.75)';
  }, true);
  document.addEventListener('mouseup', function(){
    c.style.transform = 'scale(1)';
    c.style.background = 'rgba(255,60,0,.35)';
  }, true);
  return 'installed';
})();
```

**Re inject after every navigation.** MCP exposes no `addInitScript`, only
`browser_evaluate` against the current document, so a page load destroys the
overlay. This is the same rule the `walkthrough` skill states for its own badges
and arrows, and for the same reason. Re inject after any `browser_navigate` and
after any click that causes a full page load. The snippet returns `'already'`
when it is still installed, so calling it again costs nothing and is the cheapest
way to be certain.

**What this cursor does and does not show.** Playwright's `locator.click()` moves
to the element and clicks, so the overlay **jumps** to each target rather than
gliding across the page. Do not describe the result as smooth mouse movement. It
shows where each click landed and that the press happened, which is the part that
is evidence. To make a jump readable, `browser_hover` the target first and then
click: the overlay lands, then the press animates.
...
**Step 0 — Chapter slate (skip when `RECORDING = none`).**

The recording is one long take. Without markers there is no way to find the
segment for a given test case, and no way to prove a segment shows the test it is
captioned with. The slate is both: the cut anchor Phase 7 trims on, and the label
a viewer reads.

Inject it with `browser_evaluate`, substituting the real TC id and description:

```javascript
(function(id, desc){
  var old = document.getElementById('__e2e_slate__');
  if (old) old.remove();
  var s = document.createElement('div');
  s.id = '__e2e_slate__';
  s.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:2147483646;'
    + 'background:#12333F;color:#fff;font:600 15px/1.4 system-ui,sans-serif;'
    + 'padding:10px 16px;pointer-events:none;'
    + 'box-shadow:0 2px 12px rgba(0,0,0,.35);';
  s.textContent = id + ' \u2014 ' + desc;
  document.documentElement.appendChild(s);
  return s.textContent;
})('TC-NNN', 'the TC description from plan.md');
```

Then, in this order:

1. Take the wall clock **immediately**, before waiting (`date +%s.%N`), and
   append one line to `$RECORD_DIR/timeline.tsv`, tab separated:
   `TC-NNN` `start` `<epoch seconds>` `<description>`

   The order matters and it is easy to get backwards. The builder opens its cut
   half a second before this mark, so a mark taken after the hold puts most of
   the slate ahead of the cut. Measured at the builder's default pad: marking
   after a 1.5 second hold left 0.45s of slate inside the window and the caption
   was gone before a viewer could read it. Marking first puts the whole hold
   inside the cut.
2. `browser_wait_for` about 1.5 seconds, so the slate is legible in the
   recording.
3. Remove the slate before executing any step, so it never covers an element the
   test needs to reach:
   ```javascript
   (function(){ var e=document.getElementById('__e2e_slate__'); if(e)e.remove(); })();
   ```
4. Re inject the cursor overlay from 0h-iv if this TC's first step navigates.
   Step 2 re injects it after every later navigation.

Step 5 writes this TC's matching `end` line when the test case finishes.

`timeline.tsv` is the only thing Phase 7 cuts on. A TC with no `start` line
cannot appear in the video at all, so write it before the steps run, never after.

**The slate marks the start of what the clip should show, not the start of
getting ready.** Navigation, waiting for a page to load, and any fixture setup
belong **before** the slate goes up. Put them after it and they land inside the
cut window, where a slow deployed backend stretches the segment with dead time:
one test case ran 48 seconds against 14 seconds of narration, and the frame
extracted from its middle showed a stale page rather than the thing the test was
about. If a test case genuinely needs a long setup, do that setup first, then
slate, then act.
...
**Re inject the cursor overlay after every navigation in this TC, not just the
first one (skip when `RECORDING = none`).** Any `browser_navigate`, and any click
that loads a new page, destroys the overlay. Run 0h-iv's snippet again straight
after, before the next click, on whichever step navigated. A four step TC that
navigates on step 3 and does not re inject films the rest of its own evidence
with no pointer at all. The snippet returns `'already'` when it survived, so
calling it on every step costs nothing and is the cheapest way to be certain.
...
**Then hold the box on screen before removing it (skip when `RECORDING = none`).**
`browser_wait_for` about 1.5 seconds. The screenshot needs one frame; the
recording needs long enough for a viewer to actually see what is marked. Removing
the overlay immediately after the capture leaves the video showing a red box for
two frames, which reads as a glitch rather than as the point of interest.
...
**Step 5 — Console check, then close this TC's timeline window.**

1. Call `browser_console_messages`. Record any errors.
2. Take the wall clock (`date +%s.%N`) and append the closing line to
   `$RECORD_DIR/timeline.tsv`, tab separated (skip when `RECORDING = none`):
   `TC-NNN` `end` `<epoch seconds>` `<result>`

Write it for every TC, whatever the result, and write it here rather than
remembering to later. A TC with a `start` line and no `end` line is not dropped,
which would at least be visible. Phase 7 clips it to 1.5 seconds from its slate,
so a passing 40 second test case ends up in the video as its caption and nothing
else, listed under "not in the video" as clipped.
...
**`STOP_EPOCH` is taken here, once, and nowhere else.** In this order, and the
order is the whole point:

1. **Take the wall clock first** (`date +%s.%N`) and write it to
   `$RECORD_DIR/stop_epoch`.
2. **Then** stop the recording: `browser_close` if `MODE = browser`, and on path
   `window` stop the background ffmpeg job.

**Read the clock before the thing that stops recording, never after.** On path
`context`, `browser_close` *is* the stop, and it also flushes and finalises a
multi megabyte webm, which is not instant. On path `window`, ffmpeg has to write
its moov atom. A clock read on the far side of either is later than the real end
of the recording by however long that took, and the builder derives the
recording's start as `STOP_EPOCH` minus the probed duration, so the error moves
every cut earlier by the same amount.

Observed, not theorised: a clock taken after `browser_close` on an 11 MB webm
put the first test case's cut on the sign in page while its narration described
the settings page. Exit 0, 0.05s a/v drift, no skipped segments, five narrated
segments, and entirely the wrong footage. Only opening the verify frames caught
it.

Phase 7a reads that file. It does not take the clock again, on either path.

Write it to disk rather than carrying it in the reply, because Phase 6 runs a
build and the full test suite between here and Phase 7. A clock read on the far
side of that is later than the real stop by however long Phase 6 took, and the
builder derives the recording's start as `STOP_EPOCH` minus the probed duration.
Measured on a 10 second fixture: a value 3 seconds late moved the only segment
from `2.50 -> 6.50` to `0.00 -> 3.50` and filmed the loading frame instead of the
test content, at exit 0, with every printed number still agreeing. Losing
`STOP_EPOCH` altogether costs the whole video.
...
## Phase 7: Proof Walkthrough Video (MANDATORY — the final stage)

**This is the last thing the session does.** Assemble the recording from Phase 0h
into one narrated video that shows the session doing the work, test case by test
case, then open it.

This is not a slideshow of stills. The screenshots still exist and three earlier
gates depend on them: Phase 3 Close's validation pass, Phase 5a's proof
inventory, and the evidence judge. They remain the still-frame record. The video
is what a person watches to see the feature work: the cursor landing on each
control, the field filling, the red box on the element under test, the page
responding.

**Entry gate:** Phase 5d must have completed, either path. If a gap was left
unresolved, still build the video, and say so in the closing print and in
`coverage-gaps.md`. A video that hides an unresolved gap is worse than none.

**If `RECORDING = none`** (no path worked, or `--no-video` was passed): go
straight to 7f, the still-image fallback. Do not silently produce nothing.

### 7a — Read back the clock and confirm the recording closed

Phase 5e stopped the recorder on both paths and wrote the wall clock to
`$RECORD_DIR/stop_epoch`. Read that value:

```bash
STOP_EPOCH=$(cat "$RECORD_DIR/stop_epoch")
```

**Do not take the clock again here, on either path.** 5e is the only capture.
The builder derives the recording's start as `STOP_EPOCH` minus the probed
duration, which is exact and self correcting even if the recorder took a second
to spin up, but only if that timestamp really is when recording stopped. A second
reading is later by however long Phase 6 took, which moves every cut by that gap
while all three of 7d's checks still pass.

If `stop_epoch` is missing, Phase 5e did not run. Say so and go to 7f. Do not
substitute the current time.

Then confirm the recording is readable. ffmpeg writes its moov atom on exit, so
a `window` file that was never stopped will not probe. Path `context` lands its
file in `$RECORD_DIR` as a `.webm` with a generated name.

```bash
ls -la "$RECORD_DIR"
```
If there is no recording, say so plainly and go to 7f. Never report a video that
was not captured.

### 7b — Write the narration, one entry per test case

Write `$RECORD_DIR/narration.json`, keyed by TC id:

```json
{
  "TC-001": "spoken text for this test case",
  "TC-002": "..."
}
```

Each entry says what its segment shows, in the order a viewer sees it: the
requirement clause the TC proves, the action being performed, the observed
result. Take the substance from `plan.md`'s Expected and Observed fields and
`requirements.md`'s clause text.

**Keep it proof, not narrative,** the same restraint `pr-mp4` uses. Say what the
segment shows. No account of what took several tries, nothing about what was
blocked and later unblocked, no roadmap of what is left. That belongs in
`report.md` and `coverage-gaps.md`.

Match roughly the segment's own length. The builder pads video to fit longer
narration by holding the last frame, so an over-long entry stretches a frozen
frame rather than getting cut off. Two or three sentences per test case is
usually right.

A TC with no entry is narrated from its `timeline.tsv` label, so nothing goes
silent by accident.

### 7c — Build it

```bash
python "${HOME}/.claude/scripts/qa_walkthrough_video.py" \
  --record-dir "$RECORD_DIR" \
  --out "$WORKSPACE_ABS/qa-walkthrough-[TASK_ID].mp4" \
  --stop-epoch "$STOP_EPOCH" \
  --width 1920
```

What it does, so you can read its output instead of trusting it: derives the
recording's t0, turns every `start`/`end` pair in `timeline.tsv` into a cut
window with half a second of padding either side, re encodes each window,
synthesises narration per segment with edge-tts, matches each segment's video and
audio length by holding the last frame or padding with silence, concatenates both
tracks, and encodes with `libx264 -crf 18 -preset slow`.

Two choices in there are deliberate and should not be "optimised" later. The
segments are re encoded rather than stream copied because a copy snaps the cut to
the nearest keyframe and lands it outside the slate. And `-tune stillimage` is
absent: `pr-mp4` uses it because a slideshow is near-static frames, but this is a
moving recording and that flag would starve the motion.

It writes `$RECORD_DIR/video-report.json` with every segment's in and out point,
its rendered length, a `skipped` list, and a `narration_failures` list.

**Read the `skipped` list and act on it.** Three entries mean a real problem:
- `no start mark` — Phase 3 Step 0 did not run for that TC, so it is absent from
  the video. Name those TCs in the closing print.
- `no end mark` — Step 5 did not run for that TC, so it is in the video clipped
  to 1.5 seconds instead of its real length. Name those TCs too.
- `falls outside the recording, dropped` — the timeline and the recording are
  from different runs. If every TC is dropped the builder exits with an error, as
  it should. Do not work around that by nudging timestamps until something
  appears.

**Read `narration_failures` as well.** edge-tts stayed down for that segment
after its retries, so the segment is in the video with silence over it. The video
is still valid proof and still worth handing over. Name those TCs in the closing
print rather than letting a viewer wonder why one test case says nothing.

### 7d — Verify it, three checks

Each catches something the others miss. The builder prints all three and exits
non zero on the drift check.

1. **Cut placement.** `Read` **every** frame in `$RECORD_DIR/verify-frames/`.
   One is extracted from the middle of each segment. Confirm each frame shows the
   test case its filename names. A segment cut from the wrong part of the
   recording is the failure that looks correct in every number and is visible
   only here.
2. **Audio and video agree.** The printed `a/v drift` must be under a second.
   Above that, some segment's audio and video lengths were mismatched.
3. **It is legible.** Read one frame at full size and confirm you can read the
   page text and see the red box. If not, the capture size set in Phase 0h was
   too small, which this step cannot fix.

Then confirm the narration is not silence:
```bash
ffmpeg -hide_banner -i "$WORKSPACE_ABS/qa-walkthrough-[TASK_ID].mp4" -af volumedetect -f null NUL 2>&1 | grep mean_volume
```
Speech reads around -20 dB. A `mean_volume` near -91 dB means the video has no
narration at all, whatever the file size suggests. Check `narration_failures` in
`video-report.json` before calling it a mixing fault: if every segment is listed
there, edge-tts was down for the whole run and the builder already said so.

### 7e — Open it

```bash
python "${HOME}/.claude/skills/powerpoint/scripts/pptx_env.py" open "$WORKSPACE_ABS/qa-walkthrough-[TASK_ID].mp4"
```

Print:
```
Proof walkthrough built and opened:
  Length            : [N]s over [N] test case segments
  Recording         : [context | window], [N]s captured, [N]s kept
  Narrated          : [N] / [N] segments   (mean volume [N] dB)
  A/V drift         : [N]s
  Cut placement     : [N] / [N] verify frames read and correct
  Not in the video  : [TC ids and why, or "none"]
  Video           → $WORKSPACE_ABS/qa-walkthrough-[TASK_ID].mp4
```

### 7f — Still-image fallback (only when there is no recording)

Reached when `RECORDING = none`, no recording file was produced, or every test
case fell outside the recording.

Say plainly, first, that there is no video walkthrough for this session and why.
Then build the still-image deck so the session still hands over something
inspectable. Call `Skill(skill="powerpoint")` and follow its process. Walk
`report.md`'s Evidence Index in `plan.md` order: one slide per test case carrying
the requirement clause it proves, Expected and Observed from `plan.md`, the
`TC-NNN-after.png` screenshot placed with `add_picture`, and the real variable
values from `TC-NNN-debug.json`. Then the failures, the "What Was Not Tested"
section from `coverage-gaps.md` verbatim, any unresolved-gap slides, and the
post-deploy items.

Do not draw a second red box over a screenshot that already carries one from
Phase 3 Step 4. Those are baked into the pixels on the right element and were
gated on a real bounding box at capture time. Guessing coordinates on a
slide-scaled image puts a red box on the wrong element, which is worse evidence
than no box.

Render every slide with `topdf` then `pdftoppm -jpeg`, and `Read` every rendered
image before opening the deck. Write it to
`$WORKSPACE_ABS/qa-proof-[TASK_ID].pptx`.
...
the proof walkthrough video (Phase 7) all ran before you saw this output. 5d is
