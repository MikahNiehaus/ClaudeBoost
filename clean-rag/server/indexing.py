...
def index_topic(
    ...
    # Evict ChromaDB client to free HNSW memory, then GC
    chroma_key = str(chroma_dir)
    ChromaStore.evict_cache(chroma_key)
    _gc_cleanup(f"topic:{topic}")

    elapsed = round(time.time() - start_time, 1)
    mem = _mem_mb()
    logger.info("index_topic(%s) done: %d files, %d chunks, %.1fs, RAM=%.1f MB",
                topic, files_indexed, chunks_created, elapsed, mem)
    return {
        "topic": topic,
