...
def _search_topic(
    ...
    """Search a single topic database."""
    chroma_dir = DATABASES_DIR / topic / "chroma"
    if not chroma_dir.exists():
        logger.warning("Topic database not found: %s", topic)
        return []
...
def _search_all_topics(
    ...
    """Search across all topic databases."""
    all_results: list[dict] = []

    if not DATABASES_DIR.exists():
        return []

    for topic_dir in DATABASES_DIR.iterdir():
        if not topic_dir.is_dir() or topic_dir.name.startswith("_"):
            continue
        chroma_dir = topic_dir / "chroma"
        if not chroma_dir.exists():
            continue

        results = _search_topic(query, topic_dir.name, embedder, limit, min_score)
        all_results.extend(results)
