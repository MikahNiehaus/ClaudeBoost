...
The six `--context` values and the five `--voice` values, the tolerance matrix
that sets rule strictness per context, and the cues for inferring a context when
none is named.

Read this when the writer names a context or a voice, or when you infer one. A
pass with no context named runs every rule at `blog` strength, which is the
strictest sensible default, so this file is not needed for that.

Restored from [conorbronsdon/avoid-ai-writing](https://github.com/conorbronsdon/avoid-ai-writing)
`references/patterns.md`, MIT. Section bodies are upstream's; the header is local.
