...
**mechanics** you enforce. The way in is a **config file** you supply by path
(`--style ./house.json`). It is applied by reading it; **nothing here verifies
any of it**, because the Node checker upstream uses was not vendored. The table
below records which rules upstream gates and which it treats as advisory, so the
column describes upstream, not this copy.

No `examples/` directory ships here, so a bare config name resolves to nothing.
The format is documented below; write your own file from it.
...
With no examples bundled, every bare name is unresolvable, so `--style chicago`
falls back to applying the guide from the model's own knowledge as best-effort,
labeled such as `Applying Chicago from general knowledge (not verified; no
compliance claim).`. `SKILL.md` instructs the model to print that status line and
not to reproduce the guide's text. Both are instructions to a model, not checked
rules, so treat that path as unverified too. For mechanically enforced house
style, use a real linter such as Vale; this layer does not enforce, it instructs.
...
- **`mechanics`** (object) — output rules. Upstream verifies the checkable subset
  with a Node script. **That script was not vendored here, so nothing verifies
  any of these.** Apply every row by reading the text, and say in your output
  that the mechanics pass was manual. The "hard" column below describes what
  upstream's checker does, not what runs here:

| key | values | how upstream checks it (not run here) |
|---|---|---|
...
Upstream mechanises it with the two commands below. **Neither script ships here**,
so they are the specification for what to do by hand, not commands to run:

```bash
node scripts/normalize-quotes.js draft.md --reference original.md   # not vendored
node scripts/check-style.js draft.md --config ./house.json          # not vendored
```
