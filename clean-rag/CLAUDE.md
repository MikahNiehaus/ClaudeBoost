...
- `hooks/research-record.py` (SubagentStop) stamps that record when `swiper` finishes.
...
**`swiper`** (Sonnet) is the researcher. When the gate nudges toward research on an edit, spawn it; it picks its own queries, covers depth and breadth, checks whether the thing already exists, reads the import graph, and reports with sources, a `COVERS:` line, and a `MATCH_STRATEGY:`. It hates writing code from scratch: its whole job is to find real working code (in the project, the stdlib, an installed dependency, GitHub, or StackOverflow) and hand it back exactly as found so the builder can place it, not just describe it. It does not write or edit project files itself. Writing original logic is the builder's last resort, said plainly when reached. It runs real research every time it fires (roughly 44 to 52k tokens and 2 to 8 minutes for a full pass) and does NOT guess whether a change is trivial. Spawning it foreground vs backgrounded no longer affects whether it gets stamped: `research-record.py` fires on `SubagentStop`, which the harness raises when the subagent actually finishes, and reads the report out of `last_assistant_message`. The old advice to force a foreground spawn was a workaround for `PostToolUse` on `Task`, which fires when the Task tool call RETURNS — and the harness launches subagents asynchronously, so that is launch time, before any report exists. It could not be complied with either: the Task tool takes no `run_in_background` parameter, so every spawn was async regardless and every stamp recorded an empty `COVERS:` scope. It's defined in `~/.claude/agents/` and preloads the `research-routing` skill (depth vs breadth routing, the does-this-exist check).
