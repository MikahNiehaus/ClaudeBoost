...
def main() -> int:
    ...
    mandate = (
        ...
        "   b. PARALLEL AGENT (background, non-blocking): Spawn an agent to run\n"
        f"      POST http://127.0.0.1:{port}/acquire-topic\n"
        '      {{"topic": "<technology-slug>", "category": "<category>"}}\n'
        "      This finds and indexes high-quality sources for FUTURE queries.\n"
        "      The agent only acquires and indexes. It does NOT read or summarize.\n"
        "      Do NOT wait for it. Do NOT spawn a second for the same topic.\n\n"
