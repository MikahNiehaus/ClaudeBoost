...
# ---------------------------------------------------------------------------
# Hyphenated compound detection (check_hyphenated_compounds)
# ---------------------------------------------------------------------------

def test_flags_non_blocking(mod):
    result = mod.check_hyphenated_compounds(["// non-blocking call", "// skip", "// ok"])
    assert result is not None
    assert result.rule == "hyphenated-compound"


def test_flags_hard_coded(mod):
    result = mod.check_hyphenated_compounds(["// hard-coded for now", "// revisit", "// skip"])
    assert result is not None
    assert result.rule == "hyphenated-compound"


def test_flags_new_prefixes(mod):
    # New prefixes added: pre, auto, type, thread, lazy, self
    cases = [
    ...
    for comment in cases:
        result = mod.check_hyphenated_compounds([comment, "// ok", "// skip"])
        assert result is not None, f"Expected flagging for: {comment!r}"
        assert result.rule == "hyphenated-compound"


def test_no_flag_filename_with_dashes(mod):
    # human-voice-guard.py has dashes but is a filename, not a compound adjective
    result = mod.check_hyphenated_compounds([
...
def test_no_flag_backtick_wrapped(mod):
    # backtick-wrapped identifiers should be ignored
    result = mod.check_hyphenated_compounds([
...
def test_no_flag_cli_flag_dry_run(mod):
    # --dry-run: 'run' is the suffix here, not the prefix before a letter
    result = mod.check_hyphenated_compounds([
...
def test_nudge_on_hyphenated_compound_integration():
    # Full integration: hook should nudge when content has a compound + 2 other comments
    content = """
...
"""
    ...
    assert result.returncode == 0
    assert b"comment-humanness" in result.stderr
    assert b"hyphenated-compound" in result.stderr
